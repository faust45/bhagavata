...
Player = {
  ...
  player: function() {
    return document.getElementById("PlayerLigth");
  }
}
...
function addBookmark(time) {
  AddBookmark.CreateNew.Dialog.open(time);
}


function setUserBookmarks(bookmarks) {
  var player = document.getElementById("PlayerLigth");

  try {
    player.setUserBookmarks(bookmarks);
  } catch (error) {
    console.log(error);
    //alert("error: "+error);
  }
...
function removeUserBookmark() {
  var index = Math.floor(Math.random()*user_bookmarks.length);
  var bookmarkTime = user_bookmarks[index];
  user_bookmarks.splice(index, 1);
  var player = document.getElementById("player");

  try {
    player.removeUserBookmark(bookmarkTime);
  } catch (error) {
    alert("error: "+error);
  }
}

function addPlayer(file, duration) {
  var params = toURL({
      "file": file, 
      "duration": duration,
...
function initPlayer(id, trackParams) {
   var code =
   AC_FL_RunContent("src", "js/player",
     "wmode", "transparent",
     "width", "530",
     "height", "20",
     "align", "middle",
